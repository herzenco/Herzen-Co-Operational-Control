...
The Vercel cron calls authenticated `GET /api/cron/content-automation` every minute. It runs the replacement automation scheduler, reconciles complete approved website articles into `scheduled`, publishes due items, and then dispatches the outbox. Authenticated `/api/cron/herzenco-publishing` runs the same website-only cycle for manual recovery. Both routes bypass the browser-login redirect but still require `Authorization: Bearer $CRON_SECRET`.

Before scheduling, the worker requires the Herzen Co. property, website channel, blog/article type, title, body, excerpt, URL-safe slug, SEO title, meta description, publication date, passing independent audit, SEO/AEO scores of at least 80, and recorded human approval. It derives website-specific posting instructions when they are absent. Invalid approved records move to `recovery_required` with `publication_state: failed` and explicit blockers; they are never force-published.

The webhook worker uses a 10-second timeout and makes up to five total attempts. Network failures, `429`, and `5xx` are retried with bounded exponential delays of 30 seconds, 1, 2, and 4 minutes. Other `4xx` responses are permanent. A delivery left in `sending` for more than five minutes is recovered. Audit rows retain attempt number, timestamps, response status, and sanitized error category/name; request bodies, authorization headers, secrets, and provider response bodies are not stored.
...
Migration `20260814140000_herzenco_article_pull_publishing.sql` must be applied before deployment. Production must contain exactly one active Vercel cron for `/api/cron/content-automation`; legacy content schedules remain disabled.
