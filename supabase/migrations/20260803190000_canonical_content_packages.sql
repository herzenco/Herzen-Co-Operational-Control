-- Canonical Phase 1 content packages for Herzen Co. website and LinkedIn.

create table public.content_rewrite_iterations (
  id uuid primary key default gen_random_uuid(),
  content_item_id uuid not null references public.content_items(id) on delete cascade,
  iteration integer not null check (iteration > 0),
  prior_asset jsonb not null,
  rewritten_asset jsonb not null,
...
create index content_rewrite_iterations_item_idx
  on public.content_rewrite_iterations(content_item_id, iteration desc);

create or replace function private.herzen_phase1_qa_passes(checklist jsonb)
returns boolean language sql immutable set search_path = '' as $$
  select coalesce((checklist ->> 'strategy_and_research_linked')::boolean, false)
    and coalesce((checklist ->> 'source_asset_linked')::boolean, false)
    and coalesce((checklist ->> 'delivery_asset_linked')::boolean, false)
    and coalesce((checklist ->> 'platform_format_valid')::boolean, false)
    and coalesce((checklist ->> 'seo_aeo_gate_passed')::boolean, false)
    and coalesce((checklist ->> 'posting_instructions_present')::boolean, false)
    and coalesce((checklist ->> 'independent_review_link_active')::boolean, false)
$$;
...
begin
  ...
  if new.status in ('ready_for_lupe','awaiting_tito','approved','scheduled','publishing','published') then
    ...
    if not private.herzen_phase1_qa_passes(new.qa_checklist) then
      raise exception 'All canonical Phase 1 QA checks must pass before review.';
    end if;
...
revoke all on function private.herzen_phase1_qa_passes(jsonb) from public;
revoke all on function private.validate_herzen_phase1_package() from public;

create trigger content_items_herzen_phase1_package_gate
before insert or update on public.content_items for each row
execute function private.validate_herzen_phase1_package();

alter table public.content_rewrite_iterations enable row level security;
create policy "active members read content rewrite iterations"
  on public.content_rewrite_iterations for select to authenticated
  using (exists (select 1 from public.operations_members m where m.user_id = (select auth.uid()) and m.active));
create policy "operators create content rewrite iterations"
  on public.content_rewrite_iterations for insert to authenticated
...
grant select, insert on public.content_rewrite_iterations to authenticated;

update public.automation_schedules
set configuration = configuration || '{"property_slug":"herzen-co","pair_limit":1}'::jsonb
where job_type in ('monthly_generation','weekly_review_pack','publish_day_notice','weekly_k2_refresh','audit_retry');
