...
export async function POST(request: Request) {
  ...
  if (action !== "commented") {
    const update = action === "approved"
      ...
      : { status: action === "declined" ? "cancelled" : "revision_requested", approval_state: action === "declined" ? "rejected" : "changes_requested" };
    const { error } = await supabase.from("content_items").update(update).eq("id", link.content_item_id);
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    if (action === "approved") {
      const { data: item } = await supabase.from("content_items").select("id,channel_id,publish_at").eq("id", link.content_item_id).single();
      const { data: channel } = item ? await supabase.from("content_channels").select("platform").eq("id", item.channel_id).single() : { data: null };
      if (item && channel && ["website","linkedin"].includes(channel.platform)) {
        await supabase.from("content_publish_jobs").upsert({ content_item_id: item.id, platform: channel.platform, status: "queued", scheduled_for: item.publish_at || new Date().toISOString() }, { onConflict: "content_item_id" });
      }
    }
  }
